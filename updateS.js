...
setInterval(function(){
	...
	var getUsers = "select * from demo.users where userid > '"+lastID+"' limit 1 ALLOW FILTERING";
...
if(result.rows.length == 0){
		...
		client.execute("update demo.users set email = 'em_"+uid+"' where userid = '"+uid+"'", [], function(err,result){
		request('http://192.168.10.76:3000/?type=update&uid='+uid, function (error, response, body) {
                        if(error)
